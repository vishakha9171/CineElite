  ...
  async ({ event, step }) => {
    ...
    await sendEmail({
      ...
      `<div style="font-family: Arial, sans-serif; line-height: 1.5;">
        ...
        <p>Enjoy the show! 🍿</p>
        <p>Thanks for booking with us!<br/>— CineElite Team</p>
      </div>`
    });
  }
);
  ...
  async ({ step }) => {
    ...
    const reminderTasks = await step.run("prepare-reminder-tasks", async () => {
      ...
      for (const show of shows) {
        ...
        // Extract unique user IDs from the occupiedSeats map/object values
        const userIds = [...new Set(Object.values(show.occupiedSeats))];
        if (userIds.length === 0) continue;

        // Fetch user data metadata details
    ...
    // Guard: Exit early if there are no tasks to deliver
    if (reminderTasks.length === 0) {
      return { sent: 0, message: "No reminders to send." };
    }

    // Step 2: Send reminder emails concurrently using Promise.allSettled
        ...
        reminderTasks.map((task) => 
          sendEmail({
            ...
            `<div style="font-family: Arial, sans-serif; padding: 20px;">
              ...
              <br />
              <p>Enjoy the show!<br />— CineElite Team</p>
            </div>`
          })
        )
      );
    ...
    // Step 3: Calculate summary results metrics
...
const sendNewShowNotifications = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    // Fetch all users across the system to prepare global email list distribution
    const users = await User.find({});

    // Loop through each registered user profile configuration row
    for (const user of users) {
      ...
      const body = `<div style="font-family: Arial, sans-serif; padding: 20px;">
        ...
        <br/>
        <p>Thanks,<br/>CineElite Team</p>
      </div>`;

      // Dispatch single email item safely inline 
      await sendEmail({
        to: userEmail,
        subject,
        body
