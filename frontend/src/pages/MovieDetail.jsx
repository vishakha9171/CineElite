...
function MovieDetail() {
  ...
  return (
    <div className="w-full min-h-screen bg-[#070a13] text-white pb-32 select-none overflow-x-hidden antialiased">
      ...
      <div className="relative z-10 -mt-[45vh] md:-mt-[55vh] max-w-7xl mx-auto px-4 sm:px-8 md:px-16 lg:px-36 pt-24">
        <div className="flex flex-col lg:flex-row gap-12 max-w-6xl mx-auto items-center lg:items-start">
          ...
          <div className="relative group shrink-0 max-md:scale-95">
            <div className="absolute -inset-1 bg-gradient-to-b from-primary/30 to-violet-600/10 rounded-2xl blur-xl opacity-75 group-hover:opacity-100 transition duration-700" />
            <img 
              src={image_base_url + movie.poster_path} 
              alt="" 
              className="relative rounded-2xl h-[420px] w-[280px] md:h-[480px] md:w-[320px] object-cover shadow-2xl border border-white/10" 
            />
          </div>
          ...
          <div className="relative flex flex-col gap-5 flex-1 w-full text-center lg:text-left">
            ...
            <div>
              ...
              <h1 className="text-4xl md:text-5xl font-black text-balance tracking-tight leading-tight bg-gradient-to-b from-white via-zinc-100 to-zinc-400 bg-clip-text text-transparent">
                {movie.title}
              </h1>
            </div>
            ...
            <div className="flex flex-wrap justify-center lg:justify-start gap-2">
              {movie.genres?.map((genre, idx) => (
                <span key={idx} className="px-3 py-1 rounded-md text-[11px] font-bold tracking-wide uppercase bg-zinc-900/40 border border-zinc-800/60 text-zinc-300">
                  {genre}
                </span>
              ))}
            </div>
            ...
            <div className="flex items-center justify-center lg:justify-start flex-wrap gap-4 mt-4 pt-4 border-t border-white/5">
              <button 
                ...
                className="flex items-center gap-2 px-6 py-3.5 text-xs bg-zinc-900/90 hover:bg-zinc-800 border border-zinc-800/80 text-zinc-100 rounded-xl
                 font-bold tracking-wider uppercase transition-all duration-300 cursor-pointer active:scale-95 shadow-lg"
              >
              ...
              <a 
                href="#dateSelect"
                className="px-8 py-3.5 text-xs bg-primary hover:bg-primary-dull text-white rounded-xl font-bold tracking-wider uppercase shadow-xl 
                shadow-primary/20 transition-all duration-300 cursor-pointer active:scale-95 text-center min-w-[140px]"
              >
                Buy Tickets
              </a>
                ...
                {movie.casts.slice(0, 11).map((cast, index) => {
                  ...
                  return (
                    <div key={index} className="flex flex-col items-center w-20 md:w-24 shrink-0 group">
                      <div className="relative w-16 h-16 md:w-20 md:h-20 rounded-full overflow-hidden border-2 border-zinc-800 group-hover:border-primary
                       transition-all duration-300 shadow-xl bg-zinc-900 flex items-center justify-center">
                        {cast.profile_path ? (
                          <img 
      ...
      {/* Cinematic Modal Frame for Trailer Player */}
